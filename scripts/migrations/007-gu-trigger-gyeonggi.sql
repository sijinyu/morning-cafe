-- 007-gu-trigger-gyeonggi.sql
-- gu 트리거 업데이트: 서울 + 경기도 주소 지원
-- 경기도: "경기 성남시 분당구" → "성남시 분당구", "경기 하남시" → "하남시"
...
-- 시+구 패턴
UPDATE cafes
SET gu = (regexp_match(address, '경기\S*\s+(\S+시)'))[1] || ' ' || (regexp_match(address, '경기\S*\s+\S+시\s+(\S+구)'))[1]
WHERE gu IS NULL
  AND address LIKE '%경기%'
  AND (regexp_match(address, '경기\S*\s+\S+시\s+(\S+구)'))[1] IS NOT NULL;

-- 시만 패턴 (구 없는 경기도 시)
UPDATE cafes
SET gu = (regexp_match(address, '경기\S*\s+(\S+시)'))[1]
WHERE gu IS NULL
  AND address LIKE '%경기%'
  AND (regexp_match(address, '경기\S*\s+(\S+시)'))[1] IS NOT NULL;

-- 3. 확인
SELECT gu, COUNT(*) AS cnt
FROM cafes
WHERE address LIKE '%경기%' AND gu IS NOT NULL
GROUP BY gu
ORDER BY cnt DESC;
