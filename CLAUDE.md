...
NEXT_PUBLIC_ADFIT_UNIT_LIST=     # AdFit 광고단위 ID (메인 리스트뷰 — 매체 심사용 지면, GU 값 재사용 가능)
...
50. **AdFit 광고 배너**: `AdFitBanner` (`src/components/adfit-banner.tsx`). `unit` 미설정이면 no-op, `isNativeApp()`이면 렌더 안 함 (WebView 웹광고 노출 = AdFit 정책 위반). 광고단위는 `src/lib/ad-units.ts`의 `AD_UNITS`에서 중앙 관리. 지면 5곳: `/cafe/[id]`(★네이버 유입 최다), `/cafes/[gu]`, `/guides/[slug]`, `/cafes`, 메인 **리스트뷰**(`cafe-list-view.tsx` — AdFit 매체 심사가 대표 URL에서 광고 설치를 확인하므로 필요, 2026-08 매체 심사 보류 대응). 지면 추가 시 AdFit에서 광고단위 **별도 발급** + env 추가. **지도 뷰에는 절대 달지 않는다** (North Star 훼손) — 메인 라우트여도 리스트뷰만 허용. iOS 앱에는 광고를 넣지 않는다 — GA 기준 플랫폼 100% WEB이라 수익 0인데 지도를 가리고 심사 리스크만 늘어난다.
