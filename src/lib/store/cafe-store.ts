...
const CHAIN_KEYWORDS = [
  ...
  // ponytail: 'ING'/'MEGA'/'PAIK'/'이차이'는 짧아서 부분매칭 오탐 심각(Morning/Loading/이차이나 등) → 제거.
  //   각각 '카페아이엔지'/'메가커피'/'빽다방'/'2CHAI'로 이미 커버됨.
  '로아스토어카페', '로아스토어', '2CHAI', '그라찌에', 'GRAZIE', '엔젤리너스',
  // 2026-08-11 추가 — 부산 확장 대비 영남권 로컬 프랜차이즈.
  //   모모스커피(부산 스페셜티 대표)는 블루보틀·테라로사처럼 목적지 카페라 의도적으로 제외.
  //   '어바웃'은 인디 카페명 오탐(어바웃타임 등) 우려로 공식 표기 '에이바우트'만.
  '블랙업', 'BLACKUP', '에이바우트', '홍루이젠', '핸즈커피', '다빈치커피',
  // 2026-08-24 추가 — 부산 크롤(2,434곳) 카페명 빈도 분석 기반.
  //   상위 체인은 대부분 기존 목록에 이미 존재(텐퍼센트·하삼동·카페051·블루샥 등),
  //   유일 누락이 카페마일로(부산 18+). "마일로 부전시장점" 단독 표기가 있어 '마일로'로.
  '마일로',
  // 2026-08-27 추가 — 부산 로컬 체인 (사용자 요청 + DB 표기 확인).
  //   '노스커피'는 마노스커피·루브노스커피 2곳 오탐 알려짐(부산대 로컬 체인 지점 다수가 우선).
  '어벤더치', '빡벳커피', '노스커피',
  // 2026-08-27 추가 — 부산 로컬 체인 2차 (사용자 제보, 전건 DB 실표기 검증).
  //   '동커'는 하삼동커피 등 362곳 파편 매칭이라 제외(하삼동은 기존 키워드로 커버).
  //   '히스'의 실표기는 히스피커피 → '히스피'. '와커피'는 기와커피류 소수 오탐 알려짐.
  '킹프레소', '고더커피', '그레이핍플', '히얼이즈', '해쉬커피', '무인다방',
  '자스민커피', '와커피', '아몽즈', '어랏투고', '커피쿡', '펠어커피',
  '3XL커피', '쓰리엑스라지', '커피로만', '요거스빈', '지니프레쉬', '히스피',
  '오티티프라', '맨리스커세권', '카페제곱',
] as const;
